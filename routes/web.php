...
Route::group(['middleware' => 'auth'], function ($app) use ($router) {
    ...
    $app->prefix('biaya')->middleware(['cek:Admin'])->name('biaya.')->group(function ($app) use ($router) {
        $app->get('/', 'BiayaJasaController@index')->name('index');
        $app->get('/create', 'BiayaJasaController@create')->name('create');
        $app->post('/store', 'BiayaJasaController@store')->name('store');
        $app->put('/{id}', 'BiayaJasaController@update')->name('update');
        $app->get('/{id}/edit', 'BiayaJasaController@edit')->name('edit');
        $app->get('/{id}/show', 'BiayaJasaController@show')->name('show');
        $app->get('/{id}/delete', 'BiayaJasaController@destroy')->name('destroy');
    });

    $app->prefix('jasa')->middleware(['cek:Admin,Petugas'])->name('jasa.')->group(function ($app) use ($router) {
        $app->get('/', 'JasaController@index')->name('index');
        $app->post('/getBiayaJasa', 'JasaController@getBiayaJasa')->name('getBiayaJasa');
        $app->get('/listadmin', 'JasaController@listadmin')->name('listadmin');
        $app->get('/listkeuntungan', 'JasaController@listkeuntungan')->name('listkeuntungan');
        $app->post('/store', 'JasaController@store')->name('store');
    });

    $app->resource('kategorijasa', 'KategoriJasaController')->except(['show'])->middleware(['cek:Admin']);
    $app->prefix('kategorijasa')->middleware(['cek:Admin'])->name('kategorijasa.')->group(function ($app) use ($router) {
        $router->get('/{id}/delete', 'KategoriJasaController@destroy')->name('destroy');
    });
    ...
    $app->prefix('report')->middleware(['cek:Admin,Manager'])->name('report.')->group(function ($app) use ($router) {
        ...
        $app->prefix('jasa')->name('jasa.')->group(function ($app) use ($router) {
            $app->get('/', 'ReportController@jasaPerPeriode')->name('periode');
            $app->get('/loadTable', 'ReportController@jasaPeriodeLoadTable')->name('jasa_load_table');
            $app->get('/excel', 'ExportExcelController@jasaExport')->name('jasa_export');
        });
