                            ...
                            <table class="table">
                                <tbody>
                                    <tr>
                                        ...
                                        <td>
                                            <button class="btn btn-success" id="filter-atas"><i
                    ...
                    <div class="row mt-3">
                        <div class="col-md-12">
                            <div class="table-responsive" id="loadTable">
                                @include('pages.report.penjualan.barang.table')
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.print').click(function() {
                ...
                let url =
                    `{{ url('report/penjualan/barang/print?barang=${barang}&start=${tanggal_awal}&end=${tanggal_akhir}') }}`;
            ...
            $('.excel').click(function() {
                ...
                let url =
                    `{{ url('report/penjualan/barang/excel?barang=${barang}&start=${tanggal_awal}&end=${tanggal_akhir}') }}`;
                const parseResult = new DOMParser().parseFromString(url, "text/html");
                const parsedUrl = parseResult.documentElement.textContent;
                window.open(parsedUrl, '_blank');
            });
            ...
            function loadTable(barang = "all", tanggal_awal, tanggal_akhir) {
                let url =
                    `{{ url('report/penjualan/barang/loadTable?barang=${barang}&start=${tanggal_awal}&end=${tanggal_akhir}') }}`;
                const parseResult = new DOMParser().parseFromString(url, "text/html");
                const parsedUrl = parseResult.documentElement.textContent;
