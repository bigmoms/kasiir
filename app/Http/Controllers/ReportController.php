...
class ReportController extends Controller
{
    ...
    public function jasaPerPeriode()
    {
        $kategori  = Kategori_jasa::with('jasa', 'biaya')->get();
        $biaya  = Biaya_admin::with('jasa', 'kategori')->get();
        $transaksi = Transaksi_jasa::with('kategori', 'biaya')->get();
        // $transaksi = DB::table('kategori_jasa')
        //             ->join('transaksi_jasa', 'transaksi_jasa.kategori_jasa_id', '=', 'kategori_jasa.id')
        //             ->get();
        return view($this->page . "jasa.index", compact('transaksi', 'kategori', 'biaya', $transaksi));
    }

    public function jasaPeriodeLoadTable()
    {
        $transaksi = Transaksi_jasa::with('kategori');
        if (request()->get('lanjut') == "all") {
            if (request()->get('transaksi') != "all") {
                $transaksi->where('kategori_jasa_id', request()->get('transaksi'));
            }
            $transaksi = $transaksi->whereDate('tanggal_transaksi', ">=", request()->get('start'));
            $transaksi = $transaksi->whereDate('tanggal_transaksi', "<=", request()->get('end'));
        } else {
        ...
        $transaksi = $transaksi->get();
        return view($this->page . 'jasa.table', compact('transaksi'));
    }
